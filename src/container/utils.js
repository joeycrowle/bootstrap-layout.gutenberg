...
export const getStyle = ( attributes ) => {
	return {
		backgroundImage: attributes.backgroundImage.hasOwnProperty('full') ? `url(${attributes.backgroundImage.full.url})` : null,
		backgroundSize: attributes.backgroundSize ? attributes.backgroundSize : null,
		backgroundRepeat: attributes.backgroundRepeat ? attributes.backgroundRepeat : null,
		backgroundPosition: attributes.backgroundPosition.hasOwnProperty('x') ? `${ Math.round(attributes.backgroundPosition.x * 100) }% ${ Math.round(attributes.backgroundPosition.y * 100) }%` : null,
		backgroundAttachment: attributes.backgroundAttachment ? attributes.backgroundAttachment : null,
		backgroundColor: attributes.backgroundColor ? attributes.backgroundColor : null
	}
}
